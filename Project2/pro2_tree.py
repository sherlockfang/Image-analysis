...
def objScoreperQuery(leafNode=None, idf=None, visualWord_object=None, queryDescriptorNumber=None, sumQuery=None, sumIDF=None):
	...
	for obj in range(50):
		...
		for vw in range(len(leafNode)):
			...
			# score = score + (idf[vw] * len(visualWord_object[vw][obj]) * queryDescriptorNumber[vw])
...
# TODO database ID
	# ----------------------- Score computation -----------------------------
		...
		rankObj.append(recallEvaluation(objScore,top=1, queryID=singleQuery.index))
	# a list of recall value(+:1, -:0)

# ------------------------- Recall rate computation ----------------------------------
	recallRate = numpy.sum(rankObj) / 40
	print(recallRate)
